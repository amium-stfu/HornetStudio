...
public interface IItemRetentionPolicyResolver
{
    ...
    ItemRetentionDecision Resolve(ItemBrokerMessage message, DateTimeOffset timestamp);
}
...
public sealed class DefaultItemRetentionPolicyResolver : IItemRetentionPolicyResolver
{
    ...
    /// <inheritdoc />
    public ItemRetentionDecision Resolve(ItemBrokerMessage message, DateTimeOffset timestamp)
    {
        ArgumentNullException.ThrowIfNull(message);

        if (message is ItemRemoveMessage or ItemWriteRequestMessage or ItemBrokerAckMessage)
        {
            return new ItemRetentionDecision(ShouldRetain: false, ItemRetentionMode.NotRetained, ExpiresAt: null);
        }

        if (ItemBrokerPath.Normalize(message.Path).StartsWith(ItemBrokerHealthPaths.Root + ".", StringComparison.OrdinalIgnoreCase))
        {
...
public sealed class DefaultItemPublishPolicyResolver : IItemPublishPolicyResolver
{
    ...
    {
        _ = ItemBrokerPath.Normalize(path);
