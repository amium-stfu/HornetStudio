...
The `AttachToUi` editor displays remote paths as a compact transport-aware tree and keeps the selected broker item identities in `BrokerAttachedItemPaths`. Saved legacy paths below `runtime.item_broker...shared...` are still recognized, but live received items are registered below the widget's `Mqtt` branch.
...
Published values use flat shared MQTT topics. With `BrokerBaseTopic=hornet`, the broker path `studio.default_layout.UdlClient1.m400.set` publishes `meta` JSON to `hornet/studio/default_layout/udl_client1/m400/set`, its readback value to `hornet/studio/default_layout/udl_client1/m400/set/read`, and properties such as `write` and `unit` to `hornet/studio/default_layout/udl_client1/m400/set/write` and `hornet/studio/default_layout/udl_client1/m400/set/unit`. The topic does not contain the MQTT client id, so every client reads and writes the same shared item topic in MQTT Explorer.
