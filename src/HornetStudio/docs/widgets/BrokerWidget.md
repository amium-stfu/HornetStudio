...
Published values use flat shared MQTT topics. With `BrokerBaseTopic=hornet`, the broker path `studio.default_layout.UdlClient1.m400.set.request` publishes `meta` JSON to `hornet/studio/default_layout/udl_client1/m400/set/request`, its value to `hornet/studio/default_layout/udl_client1/m400/set/request/value`, and properties such as `unit` to `hornet/studio/default_layout/udl_client1/m400/set/request/unit`. The topic does not contain the MQTT client id, so every client reads and writes the same shared item topic in MQTT Explorer.
