...
Management surface for a file-backed folder UDL client with an inline module list and module-scoped helper exposure editing.

- Manages host, port, and connection state
- Reads client configuration from `Clients/Udl/<client-id>.yaml` and observes the folder-scoped runtime instead of owning it directly
...
- Shows module-scoped exposure editing in grouped `Main`, `Bitmask`, `Settings`, and `Adjust` areas, with the bitmask area exposing one row per bit-capable channel such as `Read`, `Set`, or `Alert`
