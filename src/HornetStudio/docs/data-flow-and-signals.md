...
`HostRegistries.Data` darf oeffentliche Daten, interne Widget-Daten, Status-, Diagnose- und Broker-empfangene Items enthalten. Normale UI-Anzeigen und Runtime-Picker duerfen daraus nicht ungefiltert ueber `GetAllKeys()` lesen, sondern muessen `DataRegistryItemMetadata` bzw. `DataRegistryItemCapabilities` als Sichtbarkeitsregel verwenden.

- Normale Item-Anzeigen, Zielauswahlen, Chart-/Signalquellen und Python-Hostwert-Projektionen verwenden `DataRegistryItemCapabilities.Display`.
...
- ItemClient-MQTT-Empfangsdaten werden als `BrokerReceivedData()` registriert und bleiben unter sichtbaren `studio.<FolderName>.<ItemClientName>.<RemoteItemPath>`-Pfaden sichtbar.
