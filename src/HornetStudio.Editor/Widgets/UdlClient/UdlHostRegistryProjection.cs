...
internal sealed record UdlAttachmentProjectionInput(
    string RelativePath,
    string Alias,
...
internal sealed class UdlHostRegistryProjection : IDisposable
{
    ...
    public bool SynchronizeAttachments(IReadOnlyList<UdlAttachmentProjectionInput> attachments)
    {
        ...
        lock (_syncLock)
        {
            ...
            {
                var relativePath = TargetPathHelper.NormalizeConfiguredTargetPath(attachment.RelativePath);
    ...
    private static string NormalizeAlias(string relativePath, string alias)
    {
        ...
        return TargetPathHelper.NormalizeConfiguredTargetPath(alias);
    }
