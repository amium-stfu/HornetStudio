...
public partial class EnhancedSignalsBrowserDialogWindow : Window, INotifyPropertyChanged
{
    ...
    public EnhancedSignalsBrowserDialogWindow()
        : this(null, new FolderModel())
    {
    }

    public EnhancedSignalsBrowserDialogWindow(MainWindowViewModel? viewModel, FolderModel folder)
    {
    ...
    public static EnhancedSignalsBrowserDialogWindow ShowOrActivate(Window? owner, MainWindowViewModel? viewModel, FolderModel folder)
    {
        using var diagnosticsScope = UiResponsivenessDiagnostics.TrackBrowserDialogOpen(owner, nameof(EnhancedSignalsBrowserDialogWindow), folder.Name);
        var dialog = new EnhancedSignalsBrowserDialogWindow(viewModel, folder)
        {
            Owner = owner
        };
    ...
    public void UpdateFolderContext(FolderModel folder)
    {
        ...
        using var diagnosticsScope = UiResponsivenessDiagnostics.TrackBrowserOperation(this, nameof(EnhancedSignalsBrowserDialogWindow), nameof(UpdateFolderContext));
        _folder = folder;
        ApplyFolderContext(folder);
    }
    ...
    {
        var browserControl = this.FindControl<EnhancedSignalsControl>("EnhancedSignalsBrowserControl")
            ?? throw new InvalidOperationException("Enhanced signals browser control could not be resolved from the browser window XAML.");
    ...
    {
        Title = $"Enhanced Signals Browser - {folder.Name}";
        _browserItem.Name = "EnhancedSignalsBrowser";
        _browserItem.ControlCaption = "Enhanced Signals";
        _browserItem.BodyCaption = $"Folder enhanced signals for {folder.Name}";
    ...
    {
        var item = viewModel?.CreateItem(ControlKind.EnhancedSignals, 0, 0, 420, 240) ?? new FolderItemModel { Kind = ControlKind.EnhancedSignals };
        item.Name = "EnhancedSignalsBrowser";
        item.ControlCaption = "Enhanced Signals";
        item.BodyCaption = "Folder enhanced signals";
