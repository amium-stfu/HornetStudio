...
public partial class ControllersBrowserDialogWindow : Window, INotifyPropertyChanged
{
    ...
    public ControllersBrowserDialogWindow()
        : this(null, new FolderModel())
    {
    }

    public ControllersBrowserDialogWindow(MainWindowViewModel? viewModel, FolderModel folder)
    {
    ...
    public static ControllersBrowserDialogWindow ShowOrActivate(Window? owner, MainWindowViewModel? viewModel, FolderModel folder)
    {
        using var diagnosticsScope = UiResponsivenessDiagnostics.TrackBrowserDialogOpen(owner, nameof(ControllersBrowserDialogWindow), folder.Name);
        var dialog = new ControllersBrowserDialogWindow(viewModel, folder)
        {
            Owner = owner
        };
    ...
    public void UpdateFolderContext(FolderModel folder)
    {
        ...
        using var diagnosticsScope = UiResponsivenessDiagnostics.TrackBrowserOperation(this, nameof(ControllersBrowserDialogWindow), nameof(UpdateFolderContext));
        _folder = folder;
        ApplyFolderContext(folder);
    }
    ...
    {
        var browserControl = this.FindControl<ControllerControl>("ControllersBrowserControl")
            ?? throw new InvalidOperationException("Controllers browser control could not be resolved from the browser window XAML.");
    ...
    {
        Title = $"Controllers Browser - {folder.Name}";
        _browserItem.Name = "ControllersBrowser";
        _browserItem.ControlCaption = "Controllers";
        _browserItem.BodyCaption = $"Folder controllers for {folder.Name}";
    ...
    {
        var item = viewModel?.CreateItem(ControlKind.ControllerWidget, 0, 0, 420, 240) ?? new FolderItemModel { Kind = ControlKind.ControllerWidget };
        item.Name = "ControllersBrowser";
        item.ControlCaption = "Controllers";
        item.BodyCaption = "Folder controllers";
