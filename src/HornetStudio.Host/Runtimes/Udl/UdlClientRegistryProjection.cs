...
internal sealed class UdlClientRegistryProjection : IDisposable
{
    ...
    private void SynchronizeAttachments()
    {
        var requestedAttachments = _definition.AttachedItemPaths
            ...
            .Select(EnhancedSignalPathHelper.NormalizeConfiguredTargetPath)
        ...
        {
            var normalizedRelativePath = EnhancedSignalPathHelper.NormalizeConfiguredTargetPath(resolvedAttachment.RelativePath);
        ...
        var currentlyResolvedAttachments = resolvedAttachments
            .Select(static attachment => EnhancedSignalPathHelper.NormalizeConfiguredTargetPath(attachment.RelativePath))
        ...
        _attachmentProjection.SynchronizeAttachments(attachments);
    }
    ...
    {
        var normalizedPath = EnhancedSignalPathHelper.NormalizeComparablePath(e.Key);
        var ownedClientRoot = EnhancedSignalPathHelper.NormalizeComparablePath(GetOwnedClientRootPath());
        if (string.IsNullOrWhiteSpace(normalizedPath)
            || !normalizedPath.StartsWith(ownedClientRoot, StringComparison.OrdinalIgnoreCase)
            || normalizedPath.StartsWith(EnhancedSignalPathHelper.NormalizeComparablePath(UdlPathHelper.GetCanonicalStatusBasePath(FolderName, ClientId)), StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
    ...
    private bool TryResolveOwnedChannelEvent(DataChangedEventArgs e, out string moduleName, out string channelName)
    {
        ...
        var relativePath = EnhancedSignalPathHelper.NormalizeConfiguredTargetPath(e.Key)
