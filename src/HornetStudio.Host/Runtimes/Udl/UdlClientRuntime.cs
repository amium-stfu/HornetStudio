...
public sealed class UdlClientRuntime : IDisposable
{
    ...
    {
        var normalizedRelativePath = EnhancedSignalPathHelper.NormalizeConfiguredTargetPath(relativePath);
        var client = GetClient();
        if (client is null)
        {
            resolved = null;
    ...
    public IReadOnlyList<UdlAttachmentProjectionInput> GetAttachmentProjectionInput(string? serializedAttachedPaths)
    {
        ...
        foreach (var relativePath in attachedPaths)
        {
            ...
            attachments.Add(new UdlAttachmentProjectionInput(
                ...
                Alias: EnhancedSignalPathHelper.NormalizeConfiguredTargetPath(relativePath),
    ...
    private void ResetLiveValueSubscriptions(IHostUdlClient? client)
    {
        ...
        UdlRuntimeLiveValueStore.ClearScope(FolderName, ClientName);
    ...
    private static ItemModel? TryResolveRuntimeItemCore(IReadOnlyDictionary<string, ItemModel> roots, string normalizedRelativePath)
    {
        ...
        var segments = EnhancedSignalPathHelper.SplitPathSegments(normalizedRelativePath);
    ...
    {
        UdlRuntimeLiveValueStore.UpdateProperty(
...
public sealed record UdlClientRuntimeDiagnosticsSnapshot(
    bool IsConnected,
    string Host,
    int Port,
    int LocalPort,
