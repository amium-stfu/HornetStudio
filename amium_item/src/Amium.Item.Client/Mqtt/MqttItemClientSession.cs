...
/// </summary>
public sealed class MqttItemClientSession : IItemClientSession, IAsyncDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MqttItemClientSession"/> class.
    /// </summary>
    /// <param name="options">The MQTT client options.</param>
    /// <param name="clock">The optional timestamp source.</param>
    public MqttItemClientSession(
        MqttItemClientOptions options,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var publishItem in EnumerateItems(item))
        {
            await PublishParameterPathAsync(
                ...
                parameterName: MetaParameterName,
                value: ResolveMetaPayload(publishItem.ItemModel),
            ...
            await PublishParameterPathAsync(
                ...
                value: publishItem.ItemModel.Value,
                correlationId: correlationId,
                retained: retained,
                cancellationToken: cancellationToken).ConfigureAwait(false);

            foreach (var parameter in publishItem.ItemModel.Properties.GetDictionary())
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var value = string.Equals(normalizedParameterName, ValueParameterName, StringComparison.Ordinal)
            ? item.Value
            : string.Equals(normalizedParameterName, MetaParameterName, StringComparison.Ordinal)
                ? ResolveMetaPayload(item)
            : item.Properties[normalizedParameterName].Value;

        if (IsRedundantPublishedProperty(normalizedParameterName))
        {
            return new ItemServerAckMessage(
                Path: normalizedPath,
                Accepted: true,
                Reason: null,
    ...
    private async Task OnApplicationMessageReceivedAsync(MqttApplicationMessageReceivedEventArgs args)
    {
        ...
        if (!_topicMapper.TryMapTopic(topic, payload, out var mapping))
        {
            ReportReceiveDiagnostic($"received unmapped topic={topic}");
            return;
        }

        ReportReceiveDiagnostic($"received mapped topic={topic} clientId={mapping.ClientId} path={mapping.Path} parameter={mapping.PropertyName}");
        RemoteItems.Apply(mapping, payload);

        var timestamp = _clock.GetUtcNow();
        ItemServerMessage message = string.Equals(mapping.PropertyName, ValueParameterName, StringComparison.Ordinal)
            ? new ItemValueChangedMessage(mapping.Path, ParsePayload(payload), mapping.ClientId, null, timestamp)
            : new ItemPropertyChangedMessage(mapping.Path, mapping.PropertyName, ParsePayload(payload), mapping.ClientId, null, timestamp);

        foreach (var subscription in _subscriptions.Values)
        {
            if (ItemServerPath.Matches(subscription.Path, mapping.Path, subscription.Recursive))
            {
    ...
    {
        if (!AppContext.TryGetSwitch("Amium.Item.Server.Mqtt.ReceiveDiagnostics", out var enabled) || !enabled)
        {
            return;
        }
    ...
    private static string ResolveMetaPayload(ItemModel item)
    {
        ...
        if (meta.Value is string text)
        {
            return string.IsNullOrWhiteSpace(text) ? "{}" : text;
        }

        return JsonSerializer.Serialize(meta.Value);
    }
    ...
    {
        yield return new PublishItem(root, ItemServerItemPath.Resolve(root));
    ...
    private sealed record PublishItem(ItemModel ItemModel, string Path);
    ...
    private sealed class MqttItemSubscription : IItemSubscription
    {
        ...
        public Task HandleAsync(ItemServerMessage message, CancellationToken cancellationToken)
