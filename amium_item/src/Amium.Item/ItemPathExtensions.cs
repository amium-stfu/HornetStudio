...
public static class ItemPathExtensions
{
    ...
    public static Item Repath(this Item item, string absolutePath)
    {
        ...
        ApplyPath(item, ItemPath.Normalize(absolutePath));
        return item;
    }
    ...
    private static void ApplyPath(Item item, string absolutePath)
    {
        ...
        item.Properties["name"].Value = GetLastSegment(absolutePath);
        item.Properties["path"].Value = absolutePath;

        foreach (var parameterEntry in item.Properties.Dictionary)
        {
        ...
        foreach (var childEntry in item.Dictionary)
        {
            ...
            ApplyPath(child, ItemPath.Combine(absolutePath, childName));
        }
    }
