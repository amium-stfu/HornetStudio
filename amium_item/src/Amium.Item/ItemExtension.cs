...
{
    /// <summary>
    /// Provides serialization and cloning helpers for <see cref="Item"/> and <see cref="ItemProperty"/> instances.
    /// </summary>
    public static class ItemExtension
    {
        ...
        private static Dictionary<string, object?> BuildItemNode(Item item)
        {
            ...
            foreach (var param in item.Properties.Dictionary.Values)
            {
                parameters[param.Name] = ParseItemPropertyJson(param.ItemPropertyToJsonString());
            }
        ...
        private static Dictionary<string, object?> ParseItemPropertyJson(string itemPropertyJson)
        {
            return JsonSerializer.Deserialize<Dictionary<string, object?>>(itemPropertyJson)
                ?? new Dictionary<string, object?>();
        }

        /// <summary>
        /// Converts the specified item property to its JSON string representation.
        /// </summary>
        /// <param name="itemProperty">The item property to serialize.</param>
        /// <returns>A JSON string representing the item property metadata and value.</returns>
        public static string ItemPropertyToJsonString(this ItemProperty itemProperty)
        {
            var runtimeType = itemProperty.Value?.GetType();

            Dictionary<string, string?> payload = new Dictionary<string, string?>
            {
                { "Name", itemProperty.Name } ,
                { "Value", SerializeParameterValue(itemProperty.Value, runtimeType) },
                { "LastUpdate", itemProperty.LastUpdate.ToString() },
        ...
        public static Item Clone(this Item item)
        {
            ...
            clonedItem.Properties.SetDictionary(CloneProperties(item.Properties.GetDictionary()));
        ...
        private static ConcurrentDictionary<string, ItemProperty> CloneProperties(ConcurrentDictionary<string, ItemProperty> properties)
        {
            var clonedProperties = new ConcurrentDictionary<string, ItemProperty>();
            foreach (var entry in properties)
            {
                clonedProperties[entry.Key] = CloneItemProperty(entry.Value);
            }

            return clonedProperties;
        }

        private static ItemProperty CloneItemProperty(ItemProperty itemProperty)
        {
            var clonedItemProperty = new ItemProperty(
                itemProperty.Name,
                CloneParameterValue(itemProperty.Value),
                itemProperty.Path);

            clonedItemProperty.LastUpdate = itemProperty.LastUpdate;
            return clonedItemProperty;
        }
        ...
        private static void BuildItemFromJson(Item item, JsonElement jsonData)
        {
            ...
            if (jsonData.TryGetProperty("Parameter", out var paramElement) && paramElement.ValueKind == JsonValueKind.Object)
            {
                ...
                {
                    item.Properties[param.Name] = ItemPropertyFromJson(param.Value.GetRawText());
                }
            }
        ...
        /// <returns>An item property initialized with the name and value extracted from the JSON string.</returns>
        ...
        public static ItemProperty ItemPropertyFromJson(string json)
        {
            ...
            return new ItemProperty(name, value);
        }
