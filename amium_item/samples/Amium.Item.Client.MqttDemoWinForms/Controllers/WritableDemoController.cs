...
internal sealed class WritableDemoController : IItemServerClient, IAsyncDisposable
{
    ...
    {
        ClientId = clientId;
        _item = new ItemModel("Setpoint", 50.0, "Demo.Controls");
        _item.Properties["unit"].Value = "percent";
        _item.Properties["format"].Value = "0.0";
        _item.Properties["writable"].Value = true;
        _item.Properties["Description"].Value = "Publish a new value to the item topic to trigger an external write.";
    }
    ...
    internal async Task StartAsync(IItemServer broker, CancellationToken cancellationToken = default)
    {
    ...
    public async Task ReceiveAsync(ItemServerMessage message, CancellationToken cancellationToken = default)
    {
        ...
        if (!ItemServerValueCoercion.TryConvertForExistingValue(writeRequest.Value, _item.Value, out object? convertedValue))
        {
